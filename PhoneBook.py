...
class PhoneBook:
    ...
    def add_contact(self, contact):
        if contact not in self.contacts:
            ...
            self.logger.info(f"Контакт {contact.user.firstname} добавлен в вашу телефонную книгу. {self.tag}")
        else:
    ...
    def filter_user(self, user):
        ...
        for contact in filtered_contacts_user:
            self.logger.info(f"Контакт {contact.user.firstname} принадлежит следующей телефонной книге: {contact.user.firstname}")
    ...
    def filter_country_code(self, country_code):
        ...
        for contact in filtered_contacts_code:
            self.logger.info(f"Контакт {contact.user.firstname} имеет следующий код страны: {contact.country_code}")
